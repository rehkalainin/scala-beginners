...
object ListMetods extends App {
  ...
  def unic[A](list: List[A]): List[A] = list.foldLeft(List[A]()) {
    case (List(), el) => List(el)
    case (ls, el) if ls.contains(el) => ls
    case (ls, el) => ls :+ el
  }
  ...
  def pack[A](list: List[A]): List[List[A]] = {
    def helper(res: List[List[A]], remaining: List[A]): List[List[A]] = remaining match {
      case Nil => res
      case ls => {
        val (s: List[A], r: List[A]) = remaining span{_ == remaining.head}
        helper(res ::: List(s), r)
      }
    }
  ...
  println(unic(List(1, 2, 3, 4, 1, 2, 3, 4)))
  println(pack(List(1,2,3,1,2,3,1,2)))
}
